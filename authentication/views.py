...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if user_by_username and user_by_email and user_by_username != user_by_email:
            error = "Username and email already used."
        ...
        request.session.modified = True  # 🔥 important for mobile/session reliability

        title = 'Verify Your Account'
        message = f"Your OTP is: {otp_code}. Valid for 2 minutes."
...
def verify_otp_view(request):
    ...
    if request.method == 'POST':
        ...
        if 'resend_otp' in request.POST:
            ...
            try:
                send_otp_to_users("New OTP", f"OTP: {new_otp}", temp_user_data['email'])
            except Exception as e:
                logger.error(f"Email failed: {e}")
        ...
        # 🔥 AUTO LOGIN (fixes your login confusion completely)
        login(request, user)
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if remember_me:
                ...
                response.set_cookie('remember_me', 'yes', max_age=30 * 24 * 60 * 60)
            else:
